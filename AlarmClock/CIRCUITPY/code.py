...
def button_meridian(b):
    meridian = b['right'] - b['left'];
    meridian=meridian/2;
    meridian=meridian+b['left'];
    return meridian;
    
def button_equator(b):
    equator = b['bottom'] - b['top'];
    equator=equator/2;
    equator=equator+b['top'];
    return equator;
...
class Time_State(State):
    ...
    def tick(self, now):
        ...
        if (not update_time) or ((now - update_time) > 30):
            # Update the time
            update_time = now
            #time_string=datetime.strftime("%I:%M%p", time.localtime());
            suffix="a";
        ...
        if current_time is not None and not snooze_time:
            ...
            #print("now: ", minutes_now);
            #print("alarm: ", minutes_alarm);
            if minutes_now == minutes_alarm:
                #print("alarm should fire if armed");
                if alarm_armed:
                    #print("alarm firing");
                    change_to_state('alarm')
            else:
...
class Mugsy_State(Time_State):
    ...
    def tick(self, now):
        ...
        print("mugsy.tick: change state to time");
        change_to_state('time')

    def enter(self):
        global low_light
        print("mugsy.enter: set backlight and draw mugsy background");
        low_light = False
        pyportal.set_backlight(1.00)
...
class Setting_State(State):
    ...
    def touch(self, t, touched):
        global alarm_hour, alarm_minute, alarm_enabled, alarm_armed
        if t:
            if touch_in_button(t, self.buttons[0]):   # on
                ...
                if not self.previous_touch:
                    ...
                    if touch_in_button(t, self.buttons[3]):   # HOURS
                        ...
                        if t[1] < button_equator(self.buttons[4]):   # top half
                            increment=1;
                        else:
                            increment=-1;
                                
                        if t[0] < button_meridian(self.buttons[4]): # left half
                            # toggle 10's digit
                            increment=increment*10;
                            
                        alarm_minute = (alarm_minute + increment) % 60;
    ...
    def exit(self):
        super().exit();
        logger.debug("armed: ", alarm_armed);
...
#pyportal.touchscreen.samples=8;

smarttouch = SmartTouch();
smarttouch.setTscreen(pyportal.touchscreen);

touching=False; # indicates whether ONE touchpoint sample is TOUCHING the screen
touched=False; # flip-flop state to track first touch

while True:
	touching, valid=smarttouch.poll();
	
	if(not touching):
		touched=False; # clear the touched state on screen release
		
	if(valid):
		# we have captured a smart point
		stp=smarttouch.consume();
		touched = current_state.touch(stp, touched); # allow the STATE to set/clear the TOUCHED boolean
